...
//class MyIco extends Ico {

    function GetLargestIcon() {
        $MaxColor=0;
        $Width=0;
        $selectedIndex=0;
        for ($x=0; $x<=$this->TotalIcons();$x++) {
            $fmt = $this->GetIconInfo($x);
            if ($fmt['ColorCount'] >= $MaxColor) {
                $MaxColor=$fmt['ColorCount'];
                if ($fmt['Width'] >= $Width) {
                    $Width=$fmt['Width'];
                    $selectedIndex=$x;
                }
        ...
        return $this->GetIcon($selectedIndex);
    }

    function ListOfSizesAvailable() {
        $ret = array();
        for ($x=0; $x<=$this->TotalIcons();$x++) {
            $fmt = $this->GetIconInfo($x);
            array_push($ret, $fmt['ColorCount'] . " - " . $fmt['Width'] . "x" . $fmt['Height']);
        }
        return $ret;
    }

    function TryGetExactSizeIcon($ImageSize) {
        $MaxColor=0;
        $Width=0;
        $selectedIndex=-1;
        for ($x=0; $x<=$this->TotalIcons();$x++) {
    ...
    function GetClosestIcon($TargetSize) {
        $MaxColor=0;
        $Width=0;
        $selectedIndex=-1;
        for ($x=0; $x<=$this->TotalIcons();$x++) {
